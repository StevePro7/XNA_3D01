...
namespace MyGame
{
	public class Demo62Game1 : MyBaseGame
	{
		...
		protected override void LoadContent()
		{
			...
			models.Add(new CModel(Content.Load<Model>("Content/grass_ground"), Vector3.Zero, Vector3.Zero, Vector3.One, GraphicsDevice, false));
			...
			camera = new FreeCamera(new Vector3(0, 700, 3000), MathHelper.ToRadians(0), MathHelper.ToRadians(5), GraphicsDevice);
			...
			trees = new BillboardCross(GraphicsDevice, Content, Content.Load<Texture2D>("Content/tree_billboard"), new Vector2(800), positions);

			Vector3[] cloudPositions = new Vector3[350];
			for (int i = 0; i < cloudPositions.Length; i++)
			{
				cloudPositions[i] = new Vector3(r.Next(-6000, 6000), r.Next(2000, 3000), r.Next(-6000, 6000));
			}

			clouds = new BillboardSystem(GraphicsDevice, Content, Content.Load<Texture2D>("Content/cloud2"), new Vector2(1000), cloudPositions);
