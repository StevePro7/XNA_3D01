...
namespace MyGame
{
	public class BillboardCross
	{
		...
		void generateParticles(Vector3[] particlePositions)
		{
			...
			verts = new VertexBuffer(graphicsDevice, typeof(VertexPositionTexture), nBillboards * 8, BufferUsage.WriteOnly);
			verts.SetData<VertexPositionTexture>(particles);

			// Create and set the index buffer
			ints = new IndexBuffer(graphicsDevice, IndexElementSize.ThirtyTwoBits, nBillboards * 12, BufferUsage.WriteOnly);
